defmodule Commons.Code.Stringify do
  ...
  defp do_stringify(map) when is_map(map) do
    map |> Macro.escape |> do_stringify
  end
  defp do_stringify({:%{}, _, proplist}) do
    case Keyword.get(proplist, :__struct__) do
      nil    -> format_map(proplist)
      struct -> format_struct(
        struct |> Atom.to_string,
        proplist |> Keyword.delete(:__struct__, struct)
      )
    end
  end
  ...
  ##################
  # Map/Struct Formatting
  #
  # The following is the intended look
  # and feel of the formatting for maps/structs
  # that `stringify` will produce.
  #
  # Maps are identical to structs, just without
  # the struct name in the output.
  #
  # %MyStruct{
  #   :key1 => [
  #     foo: :bar  
  ...
  # }
  defp format_struct(struct, proplist) do
    indent!
    format_map(proplist, <<?%, struct::binary, ?\{>>)
  end
  defp format_map(proplist) do
    indent!
    format_map(proplist, <<?%, ?\{>>)
  end
  defp format_map([], acc) do
    acc <> "\n" <> tabs(unindent!) <> "}"
  end
  defp format_map([{key, value}|rest], acc) do
    case rest do
      [] -> format_map(rest, format_map_item(key, value, acc))
      _  -> format_map(rest, format_map_item(key, value, acc) <> ",")
    end
  end
  defp format_map_item(key, value, acc) do
    stringified_value = do_stringify(value)
    keystr = case Macro.to_string(key) do
      <<?:, _::binary>> = keystr -> keystr
      str                        -> str
    end
